...
const homebridge_lib_1 = require("homebridge-lib");
...
class SwitchBotPlatform {
    constructor(log, config, api) {
        ...
        this.accessories = [];
        this.version = process.env.npm_package_version || '2.9.0';
        ...
        this.generateHeaders = () => {
            ...
            const signTerm = crypto_1.default
                .createHmac('sha256', this.config.credentials?.secret)
                .update(buffer_1.Buffer.from(data, 'utf-8'))
                .digest();
        ...
        this.eve = new homebridge_lib_1.EveHomeKitTypes(api);
    ...
    async setupMqtt() {
        if (this.config.options?.mqttURL) {
            try {
                this.mqttClient = await (0, async_mqtt_1.connectAsync)(this.config.options?.mqttURL, this.config.options.mqttOptions || {});
    ...
    async setupwebhook() {
        ...
        if (this.config.options?.webhookURL) {
            ...
            try {
                ...
                this.webhookEventListener = http.createServer((request, response) => {
                    try {
            ...
            try {
                const { body, statusCode, headers } = await (0, undici_1.request)('https://api.switch-bot.com/v1.1/webhook/setupWebhook', {
                    method: 'POST',
            ...
            try {
                const { body, statusCode, headers } = await (0, undici_1.request)('https://api.switch-bot.com/v1.1/webhook/updateWebhook', {
                    method: 'POST',
                    headers: this.generateHeaders(),
                    body: JSON.stringify({
            ...
            try {
                const { body, statusCode, headers } = await (0, undici_1.request)('https://api.switch-bot.com/v1.1/webhook/queryWebhook', {
                    method: 'POST',
                    headers: this.generateHeaders(),
                    body: JSON.stringify({
                        'action': 'queryUrl',
            ...
            this.api.on('shutdown', async () => {
                try {
                    const { body, statusCode, headers } = await (0, undici_1.request)('https://api.switch-bot.com/v1.1/webhook/deleteWebhook', {
                        method: 'POST',
                        headers: this.generateHeaders(),
                        body: JSON.stringify({
