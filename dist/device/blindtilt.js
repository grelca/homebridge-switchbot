...
class BlindTilt {
    constructor(platform, accessory, device) {
        ...
        this.device = device;
        this.mappingMode = BlindTiltMappingMode.OnlyUp;
        //MQTT stuff
        ...
        this.context();
        this.config(device);
        this.mappingMode = device.blindTilt?.mode ?? BlindTiltMappingMode.OnlyUp;
        this.debugLog(`Mapping mode: ${this.mappingMode}`);
        // this is subject we use to track when we need to POST changes to the SwitchBot API
        this.doBlindTiltUpdate = new rxjs_1.Subject();
        ...
        (0, rxjs_1.interval)(this.deviceRefreshRate * 1000)
            .pipe((0, operators_1.skipWhile)(() => this.blindTiltUpdateInProgress))
            .subscribe(async () => {
    ...
    async openAPIparseStatus() {
        ...
        const [homekitPosition, homekitTiltAngle] = this.mapDeviceValuesToHomekitValues(Number(this.OpenAPI_CurrentPosition), String(this.OpenAPI_Direction));
        ...
        if (this.setNewTarget && this.OpenAPI_InMotion) {
            ...
            if (this.TargetPosition > this.CurrentPosition || (homekitTiltAngle && this.TargetHorizontalTiltAngle !== this.CurrentHorizontalTiltAngle)) {
                ...
                this.debugLog(`${this.device.deviceType}: ${this.CurrentPosition} Standby because reached position,` + ` CurrentPosition: ${this.CurrentPosition}`);
                this.PositionState = this.platform.Characteristic.PositionState.STOPPED;
                this.windowCoveringService.getCharacteristic(this.platform.Characteristic.PositionState).updateValue(this.PositionState);
                this.debugLog(`${this.device.deviceType}: ${this.CurrentPosition} STOPPED PositionState: ${this.PositionState}`);
            }
            ...
            this.debugLog(`${this.device.deviceType}: ${this.accessory.displayName} Standby because device not moving,` + ` CurrentPosition: ${this.CurrentPosition}`);
            this.TargetPosition = this.CurrentPosition;
            if (homekitTiltAngle) {
    ...
    async openAPIpushChanges() {
        ...
        const hasDifferentAndRelevantHorizontalTiltAngle = this.mappingMode === BlindTiltMappingMode.UseTiltForDirection && this.TargetHorizontalTiltAngle !== this.CurrentHorizontalTiltAngle;
    ...
    async startUpdatingBlindTiltIfNeeded() {
        ...
        if (this.TargetPosition > this.CurrentPosition || this.TargetHorizontalTiltAngle !== this.CurrentHorizontalTiltAngle) {
            ...
            this.debugLog(`${this.device.deviceType}: ${this.accessory.displayName} value: ${this.CurrentPosition},` + ` CurrentPosition: ${this.CurrentPosition}`);
        }
        else if (this.TargetPosition < this.CurrentPosition) {
            this.PositionState = this.platform.Characteristic.PositionState.DECREASING;
            this.setNewTarget = true;
    ...
    async updateHomeKitCharacteristics() {
        ...
        if (this.mappingMode === BlindTiltMappingMode.UseTiltForDirection) {
            if (this.CurrentHorizontalTiltAngle === undefined || Number.isNaN(this.CurrentHorizontalTiltAngle)) {
                ...
                this.windowCoveringService.updateCharacteristic(this.platform.Characteristic.CurrentHorizontalTiltAngle, Number(this.CurrentHorizontalTiltAngle));
                this.debugLog(`${this.device.deviceType}: ${this.accessory.displayName} 
